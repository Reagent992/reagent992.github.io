...
draft = true
title = 'Система обработки ошибок в FastAPI проекте использующем сервисный слой'
...
2. Мы не можем использовать `HTTPException` в сервисном слое т.к. сервисный слой может быть вызван не из HTTP эндпоинта, нам надо иметь возможность поймать ошибку которую возвращает сервисный слой.
...
async def create_book(
    ...
    return BookResponse.model_validate(created_book)      # Преобразование созданной книги в API ответ
                                                          # (SQLAlchemy объекта в Pydantic схему)
```

#### 1.1 Pydantic схемы для входящего и исходящего запроса
...
#### 3.1 Базовые классы для своих ошибок

- Тут используется библиотека `caseconverter` для изменения регистра текста.
...
from schemas.errors import BaseAPIErrorSchema
...
class BaseAPIError(BaseError):
    ...
    def get_schema(self: Self) -> BaseAPIErrorSchema:
        ...
        Note:
            Вызывается обработчиком ошибок, который перехватывает ошибки основанные на BaseAPIError.
        """
    ...
    def build_example(
        ...
        """Сборка OpenAPI-пример ошибки.

        Note:
          Используется в примерах ответов эндпоинта.
        """
        return {
            "summary": summary,
            "value": {
...
class BaseErrorMessages(StrEnum):
    """Базовый Enum для сообщений об ошибках.

    Позволяет сгенерировать значение для поля `type`
    используемого в `PydanticCustomError`.
    """

    @property
    def type(self) -> LiteralString:
        """Значение для поля `type` используемого в `PydanticCustomError`."""
        return cast("LiteralString", snakecase(self.name))

# Заготовка содержащая в себе HTTP код.
class HTTP409Error(BaseAPIError):
    """Ошибка с HTTP статусом 409"""

    status_code: int = status.HTTP_409_CONFLICT
```

#### 3.2 Pydantic схемы для ошибок

Используется в OpenAPI и формирование примеров ошибок.

```python
# schemas/errors.py
from pydantic import BaseModel

class BaseAPIErrorSchema(BaseModel):
    """Схема ошибки BaseAPIError."""
...
class BookISBNAlreadyExistsError(HTTP409Error):
    """Ошибка, такой isbn уже существует."""
    # Сам текст ошибки храним в Enum
    message: str = BookErrorMessages.ISBN_ALREADY_EXISTS
```
...
from exceptions.base_errors import BaseAPIError

async def base_api_error_exception_handler(
    request: Request,
    exc: BaseAPIError,
) -> Response:
    """Обработчик одиночных ошибок API.

    Note:
      Нужен чтобы явно не ловить каждую ошибку сервиса в эндпоинте.
    """
...
app = FastAPI()
app.exception_handler(BaseAPIError)(base_api_error_exception_handler)
```
